...
{
  string bin = bitset<6>(a).to_string();

  return bin;
}

string int2binSO(int a)
{
  string bin = bitset<4>(a).to_string();
...
string getViablePair(string SB_I, string SB_O, int s[4][16], int pairAmount)
{
  ...
  while(pairFound < pairAmount)
  {
    ...
    {
      cout << "pair found: SI = " << SI << " --- F_SI = " << F_SI << endl;
      return SI;
      pairFound++;
    }
